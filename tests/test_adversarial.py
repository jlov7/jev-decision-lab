...
class TransportTests(unittest.TestCase):
    def setUp(self):
        self.env=patch.dict('os.environ',{'JEV_ALLOW_LIVE':'1','TYPESAFE_API_KEY':'test-only-not-a-real-key'})
    ...
    def test_headers_endpoint_usage(self):
        reply=provider.replay('S02');reply['model']='jev-1.13.0';reply['usage']={'input_tokens':2000,'output_tokens':100}
        ...
        self.assertAlmostEqual(meta['estimated_cost_usd'],.000084)
        self.assertEqual(meta['kind'],'live_typesafe');self.assertEqual(provider.BUDGET.used,1)
    def test_live_disabled(self):
        with patch.dict('os.environ',{},clear=True):
            with self.assertRaises(PermissionError):provider.live(self.payload)
    def test_budget(self):
        b=provider.CallBudget(1);b.reserve()
        with self.assertRaises(RuntimeError):b.reserve()
    def test_invalid_budget(self):
        for n in [0,101,True,'20']:
            with self.assertRaises(ValueError):provider.CallBudget(n)
    ...
    def test_401_redacted_not_retried(self):
        opener=MagicMock();opener.open.side_effect=urllib.error.HTTPError(provider.ENDPOINT,401,'bad',{},io.BytesIO(b'test-only-not-a-real-key'))
        with patch('urllib.request.build_opener',return_value=opener):
            with self.assertRaises(RuntimeError) as e:provider.live(self.payload)
        self.assertIn('401',str(e.exception));self.assertNotIn('test-only',str(e.exception));self.assertEqual(opener.open.call_count,1)
    ...
    def test_invalid_json(self):
        connection=MagicMock();connection.__enter__.return_value.read.return_value=b'not json'
        opener=MagicMock();opener.open.return_value=connection
        with patch('urllib.request.build_opener',return_value=opener):
            with self.assertRaises(ValueError):provider.live(self.payload)
    def test_inconsistent_heads(self):
        r=provider.replay('T03');a=r['answers']['next_evidence'];a['choice']='none'
