...
Optional comparison arms use an Anthropic API key (`uv sync --extra compare`), an OpenAI API key, or the owner's installed and signed-in Claude Code command. They require explicit consent and share `JEV_MAX_COMPARE_CALLS` (default 40). API charges and a subscription's list-price equivalent are different billing bases; they are not added into one total. Read the [evaluation protocol](docs/EVALUATION.md) and [historical detailed CLI/API reference](https://github.com/jlov7/jev-decision-lab/blob/93c076302e6fdf4891e08e8d4fa71bef9c63f3a1/README.md) for the unchanged older endpoints and environment variables. Historical timing, pricing and model-availability examples are not current promises.
