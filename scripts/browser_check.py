    ...
    with sync_playwright() as p:
        ...
        def bridge(path, options):
            ...
            req = urllib.request.Request(f'http://127.0.0.1:{http.server_port}' + path,
                data=body.encode() if body else None, headers=options.get('headers', {}),
        ...
        page.add_script_tag(content="window.fetch = async (path, options = {}) => { const r = await window.labBridge(path, options); return {ok:r.ok, status:r.status, json:async()=>r.data}; };")
        ...
        page.wait_for_function("document.getElementById('route').textContent === 'Recommend a team'")
        ...
        page.wait_for_function("document.getElementById('actionResult').textContent.includes('SIMULATED_RECOMMENDATION')")
        page.locator('#approval_current').uncheck()
        page.locator('#action').click()
        page.wait_for_function("document.getElementById('actionResult').textContent.includes('HOLD')")
        ...
        page.wait_for_function("document.getElementById('route').textContent === 'Refresh the evidence'")
        ...
        page.wait_for_function("document.getElementById('gardenResult').textContent.includes('deterministic')")
        ...
        page.locator('#run').click()
        page.wait_for_function("document.getElementById('route').textContent !== 'Ready to inspect'")
...
report={'browser':'Playwright / system Chromium','fallback_reason':'Browser plugin absent; managed Chromium blocks loopback navigation. Local-asset rendering with a bridge to the real HTTP server was used; browser-network/CSP integration remains untested.',
        'viewports':[[1440,1100],[390,844]],'checks':checks,'page_errors':errors,
        'live_jev_executed':False,'status':'PASS'}
